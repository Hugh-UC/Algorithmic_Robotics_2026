...
def compute_motion_covariance(relative_pose: np.ndarray,
    ...
    """
    # Calculate Translational and Rotational Components of the Relative Pose
    delta_trans = np.sqrt(relative_pose[0]**2 + relative_pose[1]**2)
    delta_theta = np.abs(relative_pose[2])

    # Compute Variances Using the Alpha Noise Model
    var_x       = alpha1 * delta_trans**2 + alpha2 * delta_theta**2
    var_y       = alpha1 * delta_trans**2 + alpha2 * delta_theta**2
    var_theta   = alpha3 * delta_trans**2 + alpha4 * delta_theta**2

    # Construct the 3x3 diagonal covariance matrix
...
class OdometryProcessor(Node):
    ...
    def _publish_path(self, stamp):
        ...
        path_msg = Path()
        path_msg.header.stamp = stamp       # self.get_clock().now().to_msg()
        ...
        for _, pose, _ in recent:
            ps = PoseStamped()
            ps.header.stamp = stamp         # ADDED: stamp individual poses for TF synchronization
