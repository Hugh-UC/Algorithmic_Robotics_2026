...
- [🏗️ Architectural Upgrades (v2.0)](#️-architectural-upgrades-v20)
  - [1. Unified Mission Launch Architecture](#1-unified-mission-launch-architecture)
  - [2. Layered Costmaps over Bayesian Occupancy Grid](#2-layered-costmaps-over-bayesian-occupancy-grid)
  - [3. Pose Graph SLAM: Scan-to-Map Edge Matching](#3-pose-graph-slam-scan-to-map-edge-matching)
  - [4. Adaptive Pure Pursuit Lookahead](#4-adaptive-pure-pursuit-lookahead)
  - [5. Reactive Emergency Braking (Safety Shield)](#5-reactive-emergency-braking-safety-shield)
...
### 1. Unified Mission Launch Architecture
...
### 2. Layered Costmaps over Bayesian Occupancy Grid
...
### 3. Pose Graph SLAM: Scan-to-Map Edge Matching
...
### 5. Dual-Stage Safety Shield (Reactive Braking)
...
| `control.max_angular_v` | `float` | `0.18` | `0.15 - 2.0`| rad/s | Maximum turning speed. Higher values allow taking corners without braking. |
| `control.lookahead_min`| `float` | `0.2` | `0.1 - 0.4` | m | Adaptive clamp. Strict path tracking distance for tight corridors/slow speeds. |
| `control.lookahead_max`| `float` | `0.6` | `0.4 - 1.5` | m | Adaptive clamp. Sweeping corner distance for high speeds. |
| `control.lookahead_ratio`| `float`| `1.5` | `1.0 - 3.0` | - | Multiplier determining how fast lookahead scales with linear velocity. |
| `control.rate_hz` | `float` | `15.0` | `10.0 - 30.0` | Hz | Motor command publish rate. Higher = smoother arc driving. |
| `control.goal_tolerance`| `float` | `0.15` | `0.05 - 0.5` | m | Stop radius around the goal. Physical inertia requires >= 0.15m to prevent start/stop orbiting and SLAM-jitter stuttering. |
...
| `planning.smooth_weight` | `float` | `0.3` | `0.1 - 0.8` | - | "Rubber band" strength. Higher pulls jagged A* corners into sweeping curves. |
| `planning.data_weight` | `float` | `0.4` | `0.0 - 0.5` | - | Path fidelity weight. Prevents the smoother from pulling the route into walls. |
| `planning.goal_smooth_dist`| `float` | `10.0`| `0.0 - 20.0`| cells| Distance from the goal below which smoothing is disabled to allow tight, accurate final approaches. |
| `planning.smooth_tolerance` | `float` | `0.01`| `0.001 - 0.1` | - | Numerical stopping condition for the gradient descent smoother. |
| `goal.tolerance` | `float` | `0.15` | `0.05 - 0.5` | m | Distance at which the planner suspends path generation (Must match Navigator). |
...
| `costmaps.global.inflation_radius_cells` | `float`| `12.0` | `10.0 - 20.0` | cells | Bounding penalty aura for permanent walls. Forces macro-path into the center of hallways. |
...
| `costmaps.local.inflation_radius_cells` | `float`| `10.0` | `4.0 - 10.0` | cells | The penalty aura applied strictly to live dynamic laser hits. |
| `costmaps.local.inflation_weight` | `float` | `10.0` | `5.0 - 30.0` | - | Fear of dynamic objects. Kept lower than global so the robot doesn't panic. |
| `costmaps.local.max_obstacle_range` | `float` | `2.0` | `1.5 - 12.0` | m | Ignores laser hits further than this distance to save CPU during replanning. |
...
| Parameter | Type | Default | Range | Unit | Description |
| :--- | :---: | :---: | :---: | :---: | :--- |
| `slam.keyframe_distance` | `float` | `0.06` | `0.05 - 0.5` | m | Drops a graph node every 6cm (at 0.3 m/s = updates every ~0.2s). |
| `slam.keyframe_angle` | `float` | `0.05` | `0.05 - 0.5` | rad | Drops a node every ~3 degrees to perfectly track physical Create 3 base skidding. |
| `slam.map_match_weight` | `float` | `0.3` | `0.0 - 1.0` | - | Blends relative (scan-to-scan) tracking with absolute (scan-to-map) loop closure. |
| `slam.map_publish_interval`| `float` | `0.3` | `0.3 - 2.0` | s | Global SLAM map publish rate. Lowering to `0.3` repairs dynamic "ghost walls" faster. |
| `slam.scan_rate_limit` | `float` | `10.0`| `5.0 - 20.0`| Hz | Throttles incoming laser scans to prevent CPU locking. Matches the physical RPLIDAR-A1 spin rate. |
| `scan_matcher.search_theta`| `float` | `0.1` | `0.1 - 0.4` | rad | Search radius. Increase to `0.4` if the robot slips during high-speed turns. |
| `occupancy_grid.max_range` | `float` | `7.5` | `3.0 - 15.0`| m | View distance. Set equal to map size to allow SLAM to lock onto distant corners. |
