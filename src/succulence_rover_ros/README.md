...
| `control.goal_tolerance`| `float` | `0.05` | `0.05 - 0.5` | m | Stop radius around the goal. |
...
* **Symptom: The robot wobbles, overshoots, or cuts corners too aggressively (Drunk Driving).**
  * **Tweak:** Decrease `control.lookahead` (e.g., to `0.7`) or lower `control.max_linear_v`.
  * **Why:** A smaller lookahead forces the Pure Pursuit controller to track the immediate A* path more strictly instead of sweeping wide arcs.

* **Symptom: The robot stops for dynamic objects (people), but takes too long to resume when they move.**
  * **Tweak:** Decrease `slam.map_publish_interval` (e.g., to `0.5`).
  * **Why:** Forces SLAM to re-publish the global map more frequently, speeding up the Bayesian evaporation of "ghost" obstacles.
