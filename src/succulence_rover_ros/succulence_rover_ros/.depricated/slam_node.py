...
class SlamNode(Node):
    ...
    def _optimize_and_rebuild(self):
        ...
        threading.Thread(target=self._rebuild_map, daemon=True).start()
        self.get_logger().info('Optimisation complete.')

    def _rebuild_map(self):
        t0 = time.monotonic()
        
        # 1. Snapshot the data to avoid race conditions with the main thread
        poses = [p.copy() for p in self.pose_graph.get_poses()]
        scans = list(self.keyframe_scans)
        ...
        # 3. Render all keyframes into the background grid
        n_rendered = 0
        for i, (ranges, angle_min, angle_increment) in enumerate(scans):
            if i >= len(poses):
                break
            bg_mapper.update(
        ...
        # The main thread never sees an empty map, completely eliminating the flash.
